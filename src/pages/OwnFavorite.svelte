<script lang="ts">  
    ...
    const initFavoriteComponent = async() => {
        try {
            favoritesDatas.movie = await getMovies(uid);
            favoritesDatas.tv = await getTvs(uid);
            favoritesActors = await getActors(uid);
    ...
    const removeMovieToFavorite = async(index: number) => {
        ...
        await deleteMovie(documentId, uid);
